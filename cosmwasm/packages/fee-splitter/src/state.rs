...
use cosmwasm_std::{Decimal, DepsMut, CosmosMsg, BankMsg, Coin, Fraction};
use cw_storage_plus::Item;

use crate::error::FeeSplitterError;
...
const CONFIG: Item<Config> = Item::new("fee_splitter_config");

pub fn instantiate_fee_splitter(deps: DepsMut, fee_percentage: Decimal, shares: Vec<Share>) -> Result<(), FeeSplitterError> {
    ...
    edit_fee_split_config(deps, fee_percentage, shares)
}

pub fn edit_fee_split_config(deps: DepsMut, fee_percentage: Decimal, shares: Vec<Share>) -> Result<(), FeeSplitterError> {
    if fee_percentage > Decimal::percent(100) {
        return Err(FeeSplitterError::InvalidFeePercentage {})
    }
...
pub fn get_fee_split(deps: DepsMut, full_fee: Coin) -> Result<(Vec<CosmosMsg>, Coin), FeeSplitterError> {
...
mod tests {
    mod instantiate {
        ...
        use crate::state::{instantiate_fee_splitter, Share, CONFIG};
        use crate::error::FeeSplitterError;
        ...
        fn test_instatiate_fee_splitter() {
            ...
            instantiate_fee_splitter(deps.as_mut(), Decimal::permille(5), vec![dev_share, stakers_share]).unwrap();
        ...
        fn test_instantiate_with_invalid_fee_percentage() {
            ...
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::percent(101), vec![dev_share, stakers_share]).unwrap_err();
        ...
        fn test_instantiate_with_invalid_shares() {
            ...
            // No shares
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::percent(100), vec![]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // Shares add up to less than 100
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::percent(100), vec![dev_share.clone()]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // shares add up to more than 100
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::percent(100), vec![dev_share, stakers_share, someone_else_share]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});
        }
        ...
        fn test_instantiate_with_already_instantiated_config() {
            ...
            instantiate_fee_splitter(deps.as_mut(), Decimal::permille(5), vec![dev_share.clone(), stakers_share.clone()]).unwrap();
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::permille(5), vec![dev_share, stakers_share]).unwrap_err();
        ...
        fn test_instantiate_with_zero_fee_percentage() {
            ...
            let err = instantiate_fee_splitter(deps.as_mut(), Decimal::zero(), vec![dev_share, stakers_share]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // Without shares should work
            instantiate_fee_splitter(deps.as_mut(), Decimal::zero(), vec![]).unwrap();
        }
    }
    ...
    mod edit {
        ...
        use crate::{state::{Share, instantiate_fee_splitter, edit_fee_split_config, CONFIG}, error::FeeSplitterError};
        ...
        fn test_edit_config() {
            ...
            instantiate_fee_splitter(deps.as_mut(), Decimal::permille(5), vec![dev_share_before, stakers_share_before]).unwrap();
            ...
            edit_fee_split_config(deps.as_mut(), Decimal::permille(10), vec![dev_share_after, stakers_share_after]).unwrap();
            let config = CONFIG.load(&deps.storage).unwrap();
        ...
        fn test_edit_with_invalid_fee_percentage() {
            ...
            let err = edit_fee_split_config(deps.as_mut(), Decimal::percent(101), vec![dev_share, stakers_share]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidFeePercentage {});
        }
        ...
        fn test_edit_with_invalid_shares() {
            ...
            // No shares
            let err = edit_fee_split_config(deps.as_mut(), Decimal::percent(100), vec![]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // Shares add up to less than 100
            let err = edit_fee_split_config(deps.as_mut(), Decimal::percent(100), vec![dev_share.clone()]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // shares add up to more than 100
            let err = edit_fee_split_config(deps.as_mut(), Decimal::percent(100), vec![dev_share, stakers_share, someone_else_share]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});
        }
        ...
        fn test_edit_with_zero_fee_percentage() {
            ...
            // With shares should error
            let err = edit_fee_split_config(deps.as_mut(), Decimal::zero(), vec![dev_share, stakers_share]).unwrap_err();
            assert_eq!(err, FeeSplitterError::InvalidShares {});

            // Without shares should work
            edit_fee_split_config(deps.as_mut(), Decimal::zero(), vec![]).unwrap();
            let config = CONFIG.load(&deps.storage).unwrap();
    ...
    mod get_fee_split {
        ...
        use crate::state::{instantiate_fee_splitter, get_fee_split, Share};
        ...
        fn test_get_fee_split() {
            ...
            // 1000 umpwr fee
            let (fee_split_msgs, remaining_amount) = get_fee_split(deps.as_mut(), Coin {
