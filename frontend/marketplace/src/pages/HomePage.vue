...
const handleSearch = (filterValues: any) => {
  ...
  if (filterValues.price && (filterValues.price[0] || filterValues.price[1])) {
    queryBuilder.addPricePerCredit(filterValues.price[0] , filterValues.price[1]);
  }
  queryBuilder.addPagination(5, 0)
...
<template>
  <div class="p-5 md:px-[10%]">
    ...
    <template v-if="!showSpinner">
      <AuctionSection :auction-array="data?.result?.marketplaceListings?.nodes" :filter-values="filter"/>
    </template>
  </div>
</template>
