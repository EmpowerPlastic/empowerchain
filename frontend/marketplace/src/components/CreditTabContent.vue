...
<template>
  <CustomSearchBar v-model="searchTerm" placeholder="Search Credits" @search-click="handleSearch"/>
  <CustomSpinner :visible="showSpinner"/>
  <template v-if="!showSpinner">
    <CustomAlert :visible="true" :label="`${data?.result?.query?.creditBalances?.totalCount || 0} credits found`"/>
    <div v-for="credit in data?.result?.query?.creditBalances?.nodes" :key="credit.id">
      <CreditCard :card-data="credit"/>
    </div>
    <div class="flex justify-center md:justify-end my-10">
      <CustomPagination :total="data?.result?.query?.creditBalances?.totalCount" :item-per-page="itemsPerPage"
