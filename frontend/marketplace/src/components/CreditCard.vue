    ...
    <div class="hidden md:grid  md:col-span-2">
      <div class="grid-cols-4 grid gap-3 p-5">
        <div class="col-span-1">
          ...
          <p class="text-title18">{{ cardData?.creditCollection?.creditType }}</p>
        </div>
        <div class="col-span-1">
          <p class="text-title14 font-light text-textGray  mb-1">Material</p>
          <ul class="list-disc ml-6">
            <li class="text-title18"
                v-for="material in getDetailsList(cardData.creditCollection.creditData.nodes).material"
                :key="material.key">
              {{ material.value }}
            </li>
          </ul>
        ...
        <div class="col-span-2 flex flex-col justify-between text-right">
          <div>
            ...
            <p class="text-title24 text-subTextGray">{{ cardData?.creditCollection?.denom }}</p>
          </div>
          <div class="grid grid-cols-2 gap-3">
            ...
            <button
                class="btn certificate-button" @click="showRetireCreditsModal=true">
              Retire credits
            </button>
          </div>
        </div>
    ...
    <div class="md:hidden">
      <div class="grid grid-cols-2 gap-1 pt-5 px-5">
        <div>
          ...
          <p class="text-title14 font-bold">{{ cardData?.creditCollection?.creditType }}</p>
        </div>
        <div class="text-right">
          <p class="text-title24 font-bold">{{ cardData.amountActive }}</p>
          <p class="text-title14 font-light">{{ cardData?.creditCollection?.denom }}</p>
        </div>
        <div>
          <p class="text-title14 font-light">Material</p>
          <p class="text-title14 font-bold">{{ cardData.material }}</p>
          <ul class="list-disc ml-6">
            <li class="text-title14 font-bold"
                v-for="material in getDetailsList(cardData.creditCollection.creditData.nodes).material"
                :key="material.key">
              {{ material.value }}
            </li>
