...
<template>
  ...
  <div class="modal bg-modalBackground">
    <div
      ...
      <label class="btn btn-ghost btn-sm btn-circle absolute right-2 top-2" @click="closeModal">
      ...
      <div class="grid md:grid-cols-2 gap-5 mb-6 mt-2">
        <InputWithLabel v-model="retireCreditsAmount" label="How many Plastic credits do you want to retire?"
                        placeholder="10" id="input-1" :denom="denom" type="number"/>
        <InputWithLabel :model-value="availableCredits" label="Available credits in your account" placeholder="0.01"
      ...
      <RetireCreditTextArea label="Provide a name of the entity that should be visible as a retirerer"
                            sub-text="(will be visible on generated Plastic Credit certificate)" id="message-1"
                            v-model="name"/>
      <RetireCreditTextArea label="Provide additional data about the retirerer"
                            sub-text="(will be visible on generated Plastic Credit certificate)" id="message-2"
                            v-model="additionalInfo"/>

      <div class="flex flex-col md:flex-row justify-between modal-action">
        <label class="btn modal-button text-greenPrimary bg-lightGray mb-4 md:m-0" @click="closeModal">Cancel!</label>
