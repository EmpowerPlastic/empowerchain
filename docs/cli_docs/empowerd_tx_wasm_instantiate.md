...
Example:
```bash
$ empowerd tx wasm instantiate 1 '{"foo":"bar"}' --admin="$(empowerd keys show mykey -a)" \
```
  --from mykey --amount="100ustake" --label "local0.1.0" 


```
empowerd tx wasm instantiate [code_id_int64] [json_encoded_init_args] --label [text] --admin [address,optional] --amount [coins,optional]  [flags]
```
