## empowerd tx gov submit-legacy-proposal store-instantiate

Submit and instantiate a wasm contract proposal

```
empowerd tx gov submit-legacy-proposal store-instantiate [wasm file] [json_encoded_init_args] --label [text] --title [text] --description [text] --run-as [address]--unpin-code [unpin_code,optional] --source [source,optional] --builder [builder,optional] --code-hash [code_hash,optional] --admin [address,optional] --amount [coins,optional] [flags]
```
